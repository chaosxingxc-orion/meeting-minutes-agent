#!/usr/bin/env bash
# Foreground server for the registered E3 legal-state Pass-0 flight.
set -euo pipefail

if pgrep -x llama-server >/dev/null 2>&1; then
  echo "REFUSING: llama-server is already running" >&2
  pgrep -ax llama-server >&2
  exit 1
fi

export LLAMA_MTMD_FEAT_CACHE_DIR=/home/yansuqing/.cache/meeting-minutes/e3-state-audit-q4km
...
exec ./llama-server \
  ...
  -c 16384 -np 1 -fa on -ngl 999 -ctk q8_0 -ctv q8_0
