#!/usr/bin/env python3
"""Launch one frozen meeting of E-MATERIAL-RUNTIME-GATE-CI Pass-0."""
...
def load_runtime(path: Path) -> dict[str, object]:
    ...
    if document.get("schema") != "material-runtime-gate-ci-pass0-runtime-v1":
...
def main() -> int:
    ...
    head = HeadRequest(SYSTEM_INSTRUCTION_TEMPLATE, (), {}, TEMPLATE_ID, TEMPLATE_SHA256)
    args.responses_out.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="material-gate-ci-") as temporary, args.responses_out.open(
        ...
        for position, turn in enumerate(turns, start=1):
            ...
            request_id = f"ematerialci-{meeting['file_id']}-turn{int(turn['index']):04d}"
    ...
    receipt.write(
        args.receipt_out,
        repo_root=ROOT,
        run_id=f"e-material-runtime-gate-ci-{meeting['file_id']}-pass0-v1",
    )
