#!/usr/bin/env python3
"""Build the frozen reference-blind Pass0 runtime for the new surface."""
...
def build(
    ...
    *,
    ...
    if cohort.get("config_sha256") != admission_sha256:
            ...
            clips.append(
                {
                    ...
                    "request_id": f"emns-{item['item_id'].lower()}-{role}-pass0-v1",
    ...
    if len(clips) != 40:
        raise ValueError(f"expected 40 development clips, got {len(clips)}")
    runtime: dict[str, object] = {
        "schema": "material-new-surface-pass0-runtime-v1",
        "experiment_id": "E-MATERIAL-NEW-SURFACE-PASS0",
        "evidence_tier": "INDEPENDENT_NEW_SURFACE_DEVELOPMENT",
        "claim_boundary": "Reference-blind Pass0 on development clips only; no confirmation, reference, material, embedding, or Omni correction read.",
        "inputs": {
            "admission_config_sha256": admission_sha256,
