...
现阶段路线仍拆为两层，但第一层的广播式和同chunk稀疏式验证均失败。前者不收敛且严重伤害WER；
后者可以收敛，却固化模型自己提出的词形并降低一致性。第二层training-free GRPO或多模态知识
注入继续不放行。leave-one-chunk-out独立证据审计也已失败，因此output-only检索路线停止；不能
在已读结果上调阈值或直接新增模型flight。最小公司身份供给也因机会过少而失败；后续必须使用
更丰富的独立证据，并先证明其来源、精度和覆盖门。优先候选是与会议同时可得的发布稿或演示材料。
