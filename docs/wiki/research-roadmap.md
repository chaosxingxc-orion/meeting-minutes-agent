...
`E4-DISJOINT-PREV` 用795次 Pass-0 调用得到52.76% prevalence；172-cell的 `E4-DISJOINT-DIR` 因 false-hint +3.49个百分点判为有害。后续零模型安全门审计也未找到兼具覆盖、安全和 carry 收益的 evidence/recency/width 规则。因此当前固定策略不可部署，不再在同一结果上调阈值，完整确认与 E5 继续不放行。详见[实验总表](experiments/README.md)。
