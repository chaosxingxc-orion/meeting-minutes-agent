...
`E4-DISJOINT-PREV` 用795次 Pass-0 调用得到52.76% prevalence；随后172-cell的 `E4-DISJOINT-DIR` 完成正式判读。speaker 路由的 carry 指标方向略好、总体 WER 不变，但 false-hint 增加3.49个百分点并越过安全门，因此当前固定策略不可部署，完整确认与 E5 继续不放行。详见[实验总表](experiments/README.md)。
