#!/usr/bin/env bash
# G1-PATH landing: runtime identity, the text-free per-item/per-chunk receipts,
# the structural report, both chunk plans, the logs, and the operator scripts.
#
...
doc = {
    ...
    "flight": "G1-PATH pathfinder (ES2011a + IS1008a, four arms, transcribe + minutes + qa heads)",
    "registration": "docs/readiness/2026-08-19-g1-floors-preregistration.md SS5 (REGISTERED, owner GO)",
    "scope": "STRUCTURAL validation only -- no metric scoring, no result interpretation",
    "flight_ceilings": {
        "max_calls": int(env["PATH_MAX_CALLS"]),
        "max_gpu_hours": float(env["PATH_MAX_GPU_HOURS"]),
        "max_wall_hours": float(env["PATH_MAX_WALL_HOURS"]),
        "qa_cap_used": int(env["PATH_QA_CAP"]),
        "qa_cap_registered": 200,
    },
    ...
    "server_ownership": "child of the run_g1.py chunk invocation (g1_campaign.ManagedLlamaServer)",
    "model_files": [
...
# --- 3. structural report ---------------------------------------------------
"$PY" "$SP/structural.py" "$RUN_DIR" > "$LOGS/structural-report.txt" 2>&1
...
# --- 5. archive logs + chunk plans ------------------------------------------
cp "$LOGS/preflight.log"                   "$ARCHIVE_DIR/preflight.log"
cp "$LOGS/chunkplan-registered-cap.json"   "$ARCHIVE_DIR/chunkplan-registered-cap.json"
...
cp "$SP/monitor.sh"     "$ARCHIVE_DIR/script-monitor.sh"
