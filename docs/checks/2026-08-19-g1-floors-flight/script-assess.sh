#!/usr/bin/env bash
# Point-in-time state of the G1-FLOORS campaign. Usage: assess.sh <chunk-index>
set -u
...
echo "--- item receipts ---"
echo "count: $(ls "$RUN_DIR/receipts" 2>/dev/null | wc -l) of 72"
echo "--- chunk receipts ---"
...
echo "--- stop-file ---"
echo "$YIELD_FILE present: $( [ -e "$YIELD_FILE" ] && echo YES || echo no )"
echo "--- runner log tail ---"
tail -20 "$LOGS/runner-chunk$N.log" 2>/dev/null
echo "--- progress tail ---"
