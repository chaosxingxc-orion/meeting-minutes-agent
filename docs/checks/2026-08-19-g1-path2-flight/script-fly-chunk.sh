#!/usr/bin/env bash
# G1-PATH2: ONE chunk = ONE scripts/run_g1.py --run-chunk invocation, which
...
#
# QA head at the REGISTERED N=200/seed=20260818 cap -- NO --qa-cap override:
# the 8aedcb9 per-meeting routing fix is what this re-run validates live.
#
...
echo "=== G1-PATH2 chunk $N start $(date -u +%Y-%m-%dT%H:%M:%SZ) ==="
...
# text itself is never read (PATH2 renders no metric verdict).
(
  while true; do
...
"$PY" "$SP/structural.py" "$RUN_DIR" "$LOGS/chunkplan-flight.json" 2>&1 | tee -a "$PROGRESS"
...
echo "=== G1-PATH2 chunk $N end $(date -u +%Y-%m-%dT%H:%M:%SZ) state=$STATE ==="
