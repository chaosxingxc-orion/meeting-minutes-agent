#!/usr/bin/env bash
# G1-PATH2 landing: runtime identity, the text-free per-item/per-chunk
...
doc = {
    ...
    "flight": "G1-PATH2 pathfinder re-run (ES2011a + IS1008a, four arms, transcribe + minutes + qa heads) after the 8aedcb9 QA-routing + gpu-accounting fix",
    ...
    "flight_ceilings": {
        ...
        "qa_cap_note": "REGISTERED cap, per-meeting routed (8aedcb9); no --qa-cap override",
    },
...
cp "$LOGS/pytest-preflight.log"            "$ARCHIVE_DIR/pytest-preflight.log"
...
cp "$SP/preflight-pytest.sh" "$ARCHIVE_DIR/script-preflight-pytest.sh"
