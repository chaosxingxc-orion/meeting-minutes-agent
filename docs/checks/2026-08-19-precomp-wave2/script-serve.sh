#!/usr/bin/env bash
# Foreground llama-server, started as a CHILD of the invocation wrapper (setsid/nohup do
# not survive on this machine, and a child dies with its wrapper -- which is exactly the
# teardown guarantee the ~60 min harness reap needs). Same pinned binary/flags as the
# flown P-ATTR / P-PROMPT / PRECOMP wave-1 flights: -c 49152 -np 1 -fa on -ngl 999
# -ctk q8_0 -ctv q8_0.
set -euo pipefail
