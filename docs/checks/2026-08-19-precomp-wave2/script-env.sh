#!/usr/bin/env bash
# Shared environment for the PRECOMP wave-2 production pass (2026-08-19 night batch).
# Byte-for-byte the wave-1 identity pins (docs/checks/2026-08-19-precomp-wave1/script-env.sh):
# same pinned llama.cpp build, same q4km GGUF pair, same WARM per-dataset feature cache
# ami-q4km, same pinned Arm B diar tool. Only the wave number, out-dir, and the
# per-invocation stop-file plumbing differ.
...
export SP="$SCRATCH/precomp2"
export LOGS="$SP/logs"

# The COORDINATOR's yield file: honored, never created or deleted by this operator.
export YIELD_FILE="$SCRATCH/PRECOMP_YIELD"
# This operator's OWN per-invocation stop file, handed to --stop-file. Created by the
# invocation timer (or by the yield bridge when YIELD_FILE appears) and cleared at the
# start of every invocation. Keeping them separate is what lets the wrapper end its own
# ~40 min slice without ever touching the coordinator's file.
export MY_STOP="$SP/INVOCATION_STOP"

export RUN_DIR="$DATA/derived/meeting-minutes/precomp/runs/2026-08-19-precomp-wave2"
export OUT_DIR="$REPO/docs/checks/2026-08-19-precomp-wave2"
...
mkdir -p "$LOGS" "$RUN_DIR" "$OUT_DIR/receipts" "$OUT_DIR/transport-receipts"
